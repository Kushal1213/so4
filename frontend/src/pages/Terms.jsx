import { Link } from 'react-router-dom'
import { MoonStars } from '@phosphor-icons/react'

export default function Terms() {
  return (
    <div className="grain min-h-[100dvh] bg-night-950 px-4 py-12 text-white lg:px-8">
      <div className="mx-auto max-w-2xl">
        <Link to="/" className="mb-10 inline-flex items-center gap-2 text-sm text-white/55 hover:text-white">
          <MoonStars size={16} weight="fill" className="text-moon-300" />
          Sleep Oracle
        </Link>
        <h1 className="font-display text-4xl font-semibold tracking-tight">Terms of use</h1>
        <p className="mt-2 text-sm text-white/45">Last updated March 5, 2026</p>
        <div className="mt-8 space-y-5 text-white/70 leading-relaxed text-pretty">
          <p>
            Sleep Oracle provides informational sleep screening and coaching tools. Outputs are not diagnoses, prescriptions, or medical advice.
          </p>
          <p>
            You are responsible for how you use recommendations. Seek a qualified clinician for medical concerns, sleep disorders, or treatment decisions.
          </p>
          <p>
            The service is provided as-is for demonstration and research workflows. Operators of a deployment may add additional terms for enterprise or clinical use.
          </p>
        </div>
        <Link to="/" className="mt-10 inline-block text-sm font-medium text-moon-300 underline-offset-4 hover:underline">
          Back to home
        </Link>
      </div>
    </div>
  )
}
